...
export default function PriceChart({ data }: { data: PricePoint[] }) {
  const formatted = data.map((d) => ({
    time: new Date(d.timestamp).toLocaleString("en-CA", { weekday: "short", hour: "numeric" }),
    price: d.price_toronto,
  }));

  return (
    <div className="border border-hairline bg-surface p-6">
      <p className="text-xs uppercase tracking-widest text-muted">Last 7 days · Toronto zone</p>
