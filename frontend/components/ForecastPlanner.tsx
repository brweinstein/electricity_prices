  ...
  return (
    <section className="border border-hairline bg-surface p-6 sm:p-8">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span className="border border-hairline px-2 py-1 text-xs text-muted">baseline estimate</span>
      </div>
      ...
      <form className="mt-6 grid gap-4 sm:grid-cols-[1fr_auto_auto] sm:items-end" onSubmit={handleSubmit}>
        ...
        <label className="text-sm text-muted">
          Window
          <select
            className="theme-control mt-2 block w-full border border-hairline bg-transparent px-3 py-2 text-sm text-ink outline-none focus:border-cheap"
            value={windowHours}
        ...
        <div className="mt-8 border-t border-hairline pt-6">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-1 font-mono text-5xl tabular-nums text-cheap">{forecast.average_price.toFixed(1)}<span className="ml-2 text-base text-muted">¢/kWh</span></p>
            </div>
          ...
          <div className="mt-5 h-40 border border-hairline px-2 py-3">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={forecast.estimates}>
                ...
                <YAxis stroke="#8A948F" fontSize={10} tickLine={false} axisLine={false} width={32} tickFormatter={(value) => `${value}¢`} />
                <Tooltip contentStyle={{ background: "#171D1A", border: "1px solid #2A322E", fontSize: 11 }} labelFormatter={(value) => new Date(value).toLocaleString("en-CA", { weekday: "short", hour: "numeric" })} formatter={(value) => [`${Number(value).toFixed(1)}¢`, "Estimated"]} />
